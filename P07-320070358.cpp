...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Instancia del coche 
		posx = mainWindow.getmuevex();
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f + posx, -0.2f, 4.0f));
		...
		/*faro
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-7.0f, 2.5f, 7.1f));
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Porsche2014Llanta_M.RenderModel();
		*/
		...
		// Luz en el faro 
		glm::vec3 faroLight = glm::vec3(-7.0f, 2.5f, 7.1f);
		faroLight.x += posx;
		spotLights[2].SetFlash(faroLight, glm::vec3(-1.0f,0.0f,0.0f));
